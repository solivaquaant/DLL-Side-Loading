...
import sys
import os
import time # Added for monitoring loop delay
from datetime import datetime
from logger_setup import logger, LOG_FILE
import static_analyzer
import process_analyzer
import registry_analyzer
import utils # For config loading and other utilities
from colorama import init, Fore, Style
import psutil
import tkinter as tk # Added for Message Box
...
# Added: Function to show a message box
def show_alert_messagebox(title, message, type="warning"):
    """Displays a message box alert to the user."""
    # Tkinter requires a root window, but we can hide it
    root = tk.Tk()
    root.withdraw() # Hide the main window

    # Ensure the message box is displayed on top
    ...
    # Destroy the Tkinter root window after the message box is closed
    try:
        root.destroy()
    except tk.TclError:
...
def quarantine_file(file_path):
    ...
    NOTE: This is a sensitive operation. Use with caution.
    For now, it will just log and print.
    """
    quarantine_dir = "quarantine"
    if not os.path.exists(quarantine_dir):
        try:
            os.makedirs(quarantine_dir)
            logger.info(f"Created quarantine directory: {quarantine_dir}")
        except OSError as e:
            logger.error(f"Could not create quarantine directory {quarantine_dir}: {e}")
            alert_user(f"Failed to create quarantine directory for {file_path}", "ERROR")
            return

    if os.path.exists(file_path):
        base_name = os.path.basename(file_path)
        quarantined_name = f"{base_name}_{datetime.now().strftime('%Y%m%d%H%M%S')}.quarantined"
        destination = os.path.join(quarantine_dir, quarantined_name)
        try:
            # import shutil # Import shutil if you uncomment the move
            # shutil.move(file_path, destination) # Uncomment to actually move
            logger.warning(f"ACTION (Simulated): File '{file_path}' would be moved to '{destination}' (quarantined).")
            alert_user(f"File '{file_path}' identified for quarantine. (Simulated move to {destination})", "WARNING")
        except Exception as e:
            logger.error(f"Failed to quarantine file {file_path}: {e}")
            alert_user(f"Failed to quarantine file {file_path}: {e}", "ERROR")
    else:
        logger.warning(f"Attempted to quarantine non-existent file: {file_path}")
...
def terminate_process(pid, reason=""):
    ...
    try:
        ...
        proc_name = proc.name()
        # --- BỎ COMMENT CÁC DÒNG DƯỚI ĐÂY ĐỂ KÍCH HOẠT CHỨC NĂNG KILL THỰC TẾ ---
        proc.terminate()
        proc.kill() # Mạnh mẽ hơn nếu terminate không hoạt động
        # ----------------------------------------------------------------------
        action_message = f"ACTION (Simulated): Process '{proc_name}' (PID: {pid}) would be terminated."
        if reason:
        ...
        # Không alert_user ở đây để tránh spam cho các tiến trình thoát nhanh
...
# Added: Function to terminate a process and its parent
def terminate_process_and_parent(child_pid, parent_pid, reason=""):
    ...
    # Chấm dứt tiến trình con trước
    terminate_process(child_pid, reason=f"Child of suspicious chain. {reason}")

    # Chấm dứt tiến trình cha
    if parent_pid is not None and parent_pid != 0: # Tránh chấm dứt tiến trình hệ thống 0 hoặc None
        # Thêm kiểm tra để tránh chấm dứt các tiến trình hệ thống quan trọng như services.exe, wininit.exe, v.v.
        try:
            parent_proc = psutil.Process(parent_pid)
            parent_name = parent_proc.name().lower()
            # Danh sách các tiến trình hệ thống quan trọng KHÔNG được chấm dứt
            critical_system_processes = ["services.exe", "wininit.exe", "smss.exe", "csrss.exe", "lsass.exe", "winlogon.exe", "explorer.exe"] # Thêm nếu cần
...
def print_process_scan_results(results):
    ...
    for finding in results:
        if finding["type"] == "Suspicious Process Chain":
            ...
            # Action: Terminate both child (svchost) and parent (gup)
            terminate_process_and_parent(svchost.get('pid'), gup.get('pid'), reason="Suspicious GUP.exe -> svchost.exe chain")
             ...
             alert_user(message, "CRITICAL") # Changed severity to CRITICAL for automatic termination
             show_alert_messagebox("MALICIOUS PROCESS DETECTED", message, "error") # Changed type to error for automatic termination
             print(f"{Fore.RED}{Style.BRIGHT}[ALERT] {message}") # Changed color to RED
             ...
             # Action: Terminate svchost and its unusual parent
             terminate_process_and_parent(svchost.get('pid'), parent.get('pid'), reason="Suspicious svchost parent")


        elif finding["type"] == "ProcessDetails": # For single PID scan
            color = Fore.GREEN
            ...
            elif isinstance(dlls_data, str): # For error messages like "Access Denied"
            ...
            if isinstance(connections_data, list) and connections_data:
                ...
                for conn in connections_data[:3]: # Sample
                     laddr_str = f"{conn.get('laddr',())[0]}:{conn.get('laddr',())[1]}" if conn.get('laddr') else "N/A"
                     raddr_str = f"{conn.get('raddr',())[0]}:{conn.get('raddr',())[1]}" if conn.get('raddr') else "N/A"
                     print(f"    LADDR: {laddr_str}, RADDR: {raddr_str}, Status: {conn.get('status','N/A')}")
                if len(connections_data) > 3: print("    ...")
            elif isinstance(connections_data, str): # For error messages
...
def print_all_dlls_results(results):
    ...
    for item in results[:10]: # Print for first 10 processes as an example
        ...
        if isinstance(dlls_data, list):
            if dlls_data:
                ...
                for dll_path in dlls_data[:5]: # Print first 5 DLLs
                    print(f"    - {dll_path}")
                if len(dlls_data) > 5:
            ...
            print(f"  DLLs: {dlls_data}") # e.g., "Access Denied"
    if len(results) > 10:
...
def monitor_processes(vt_api_key):
    ...
    # Populate initial known processes
    ...
    while True:
        ...
        # Identify current and new processes
        for proc in psutil.process_iter(['pid', 'name', 'exe', 'ppid']):
            try:
                ...
                if pid not in known_pids:
                    ...
                    known_pids.add(pid) # Add new process to known list immediately
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                continue # Skip processes that might have just exited or are inaccessible

        # Remove exited processes from known_pids (optional, but keeps the set cleaner)
        # known_pids = known_pids.intersection(current_pids) # This can be resource intensive
                ...
                try:
                    ...
                    if name.lower() == "svchost.exe":
                        if parent_name.lower() not in process_analyzer.STANDARD_SVCHOST_PARENTS:
                             ...
                             alert_user(message, "CRITICAL") # Changed severity to CRITICAL
                             show_alert_messagebox("MALICIOUS PROCESS DETECTED", message, "error") # Changed type to error
                             # Action: Terminate svchost and its unusual parent immediately upon detection
                             terminate_process_and_parent(pid, parent_pid, reason="Suspicious new svchost parent")


                    # Check new process file with VirusTotal (only if not already terminated by unusual parent check)
                    # Ensure process still exists before checking VT
                    if not is_suspicious_svchost_parent and psutil.pid_exists(pid) and exe_path and vt_api_key:
                        ...
                        if vt_status == "malicious":
                            ...
                            # Decide termination action: If it's a malicious svchost with suspicious parent, kill both. Otherwise, just kill the malicious process.
                            # Đặc biệt xử lý trường hợp GUP.exe spawn svchost.exe độc hại
                            ...
                            # Note: If it's a malicious svchost with a non-GUP suspicious parent, it's already handled above by is_suspicious_svchost_parent check
                            else:
                            ...
                            # No automatic termination for suspicious, requires manual review
...
def main():
    ...
    # Load config and potentially override VT API key
    vt_api_key_to_use = args.vt_api_key or utils.VT_API_KEY # Prioritize command-line arg

    use_virustotal_in_scan = args.use_virustotal or (args.full_scan and args.use_virustotal) or args.monitor # VT used in monitor mode too
    ...
    if use_virustotal_in_scan:
        if not vt_api_key_to_use:
            ...
            # If VT is required for the selected mode (monitor), exit if key is missing
            if args.monitor:
                 sys.exit(1)
        else:
    ...
    # Check if any action argument is provided
    ...
    if args.scan_processes:
        # Combined results from both process analysis functions
        process_findings = []
    ...
    if args.full_scan:
        ...
        # Combined process analysis in full scan
        process_findings = []
        process_findings.extend(process_analyzer.analyze_all_processes()) # GUP.exe -> svchost.exe pattern
        process_findings.extend(process_analyzer.analyze_svchost_parents()) # Suspicious svchost parents
        print_process_scan_results(process_findings)
